...
function setPlaySVG(isPlaying){
  el.playIcon.innerHTML = isPlaying ? '<path d="M6 5h4v14H6V5zm8 0h4v14h-4V5z"/>' : '<path d="M8 5v14l11-7L8 5z"/>';
}
...
function stop(){ el.audio.pause(); el.audio.currentTime=0; setPlaySVG(false); }

el.grid.addEventListener('click', e=>{ const card = e.target.closest('.card'); if(!card) return; play(+card.dataset.i); });
el.playBtn.addEventListener('click', ()=>{ if(currentIndex<0){ play(0); return; } if(el.audio.paused){ play(currentIndex); } else { el.audio.pause(); setPlaySVG(false); } });
...
// Media Session: prev/next/play/pause/stop shown in system notification
function updateMediaSession(s){
  if('mediaSession' in navigator){
    navigator.mediaSession.metadata = new MediaMetadata({ title: s.name, artist: 'Radio Daniel' });
    navigator.mediaSession.setActionHandler('previoustrack', ()=>{ if(currentIndex<0){select(0);return;} play((currentIndex-1+STATIONS.length)%STATIONS.length); });
    navigator.mediaSession.setActionHandler('nexttrack', ()=>{ if(currentIndex<0){select(0);return;} play((currentIndex+1)%STATIONS.length); });
    navigator.mediaSession.setActionHandler('play', ()=>{ if(el.audio.paused) play(currentIndex); });
    navigator.mediaSession.setActionHandler('pause', ()=>{ el.audio.pause(); setPlaySVG(false); });
